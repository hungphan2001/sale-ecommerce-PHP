                ...
                @foreach ($productsBestSeller as $product)
                    ...
                    <div class="product__item">
                        ...
                        <div class="product__item__text">
                            <h6>{{ $product->name }}</h6>
                            @php
                                $price = $product->discount ? get_price_sale($product->discount, $product->price, false)
                                                            : $product->price
                            @endphp
                            <button class="add-cart" data-id="{{ $product->id }}" data-price="{{ $price }}">+ Thêm vào giỏ hàng</button>
                            @if(!$product->discount)
                                <h5>
                            ...
                            @else
                                <h5>
                                    ...
                                    <span style="font-weight: normal; text-decoration: line-through; font-size: 14px">{{ number_format($product->price) . ' ₫' }}</span>
                                </h5>
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="col-lg-3 col-md-6 col-sm-6 col-md-6 col-sm-6 mix new-arrivals" style="display: none">
                            <div class="product__item">
                                ...
                                <div class="product__item__text">
                                    ...
                                    @php
                                        $price = $productNew->discount ? get_price_sale($productNew->discount, $productNew->price, false)
                                                                    : $productNew->price
                                    @endphp
                                    <button class="add-cart" data-id="{{ $productNew->id }}" data-price="{{ $price }}">+ Thêm vào giỏ hàng</button>
                                    @if(!$productNew->discount)
                                        <h5>
                                            {{ number_format($productNew->price) . ' ₫' }}
                                        </h5>
                                    @else
                                        <h5>
                                            {{ get_price_sale($productNew->discount, $productNew->price). '₫' }}
                                            <span style="font-weight: normal; text-decoration: line-through; font-size: 12px">{{ number_format($productNew->price) . ' ₫' }}</span>
                                        </h5>
                                    @endif
                                </div>
                            </div>
                    ...
                    <div class="col-lg-3 col-md-6 col-sm-6 col-md-6 col-sm-6 mix hot-sales" style="display: none">
                        <div class="product__item">
                            ...
                            <div class="product__item__text">
                                ...
                                @php
                                    $price = $productSale->discount ? get_price_sale($productSale->discount, $productSale->price, false)
                                                                : $productSale->price
                                @endphp
                                <button class="add-cart" data-id="{{ $productSale->id }}" data-price="{{ $price }}">+ thêm vào giỏ hàng</button>
                                @if(!$productSale->discount)
                                    <h5>
                                        {{ number_format($productSale->price) . ' ₫' }}
                                    </h5>
                                @else
                                    <h5>
                                        {{ get_price_sale($productSale->discount, $productSale->price). '₫' }}
                                        <span style="font-weight: normal; text-decoration: line-through; font-size: 12px">{{ number_format($productSale->price) . ' ₫' }}</span>
                                    </h5>
                                @endif
            ...
            <div class="row">
                ...
                <div class="col-lg-4">
                    <div class="instagram__text">
                        ...
                        <h3 data-href="https://www.instagram.com/tailoshop/" id="btn-instagram" style="cursor: pointer">#Male_Fashion</h3>
                    </div>
                </div>
            </div>
        </div>
